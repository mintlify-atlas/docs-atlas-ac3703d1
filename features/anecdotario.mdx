...
Parents access this route through their child's dashboard at `/mis-hijos/:estudianteId`. The `PadreAnecdotarioRepository` fetches entries from `/padre/mis-hijos/:estudianteId/anecdotarios`, which returns only the entries belonging to that student and scoped to the parent's tenant.
